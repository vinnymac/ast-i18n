    ...
    root
      ...
      .forEach((path) => {
        ...
        if (j.Identifier.check(exportDeclaration)) {
          ...
          let hookFound = addUseHookToFunctionBody(j, functions);

          if (!hookFound) {
            hocUsed = true;
            path.node.declaration = withTranslationHoc(
              j,
              j.identifier(exportDeclaration.name)
            );
            const classDeclaration = root
              .find(
                j.ClassDeclaration,
                (n) => n.id.name === exportDeclaration.name
              )
              .nodes()[0];
            if (classDeclaration) {
          ...
          exportDeclaration.arguments.forEach((identifier) => {
            const functions = findFunctionByIdentifier(
              j,
              identifier.name,
              root
            );
...
function findFunctionByIdentifier(
  j: JSCodeshift,
  identifier: string,
  root: Collection<any>
) {
  return root.find(j.Function).filter((p: NodePath<Function>) => {
...
function translateJsxContent(j: JSCodeshift, root: Collection<any>) {
  let hasI18nUsage = false;
  root.find(j.JSXElement).forEach((n: NodePath<JSXElement>) => {
    ...
    for (let i = 0; i < jsxContentNodes.length; i++) {
      ...
      if (text.trim().length > 0) {
        hasI18nUsage = true;
        newChildren.push(
          buildTranslationWithArgumentsCall(j, translateArgs, text.trim())
        );
      }
      text = '';
      translateArgs = [];
    ...
    if (newChildren.length > 0) {
      ...
      n.replace(
        j.jsxElement(n.node.openingElement, n.node.closingElement, newChildren)
      );
    }
  });
  ...
  root
    .find(j.JSXText)
    .filter(
      (path: NodePath<JSXText>) => path.node.value && path.node.value.trim()
    )
...
function translateJsxProps(j: JSCodeshift, root: Collection<any>) {
  ...
  root
    ...
    .find(j.JSXAttribute)
    .filter((path: NodePath<JSXAttribute>) =>
      hasStringLiteralJSXAttribute(path)
    )
...
function buildTranslationWithArgumentsCall(
  j: JSCodeshift,
  translateArgs: any,
  text: string
) {
...
function withTranslationHoc(j: JSCodeshift, identifier: any) {
  return j.callExpression(
    j.callExpression(j.identifier('withTranslation'), []),
    [identifier]
  );
}
